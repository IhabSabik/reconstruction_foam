...
voxels = io.imread('inputs/labeled_pores.tif')

dist_path = 'outputs/computed_distances.txt'

f = open(dist_path, 'a')
f.close()
log_file = open('outputs/log_file.txt', 'a+')
log_file_detailed = open('outputs/log_file_detailed.txt', 'a+')

with open('inputs/Centroids.txt', 'r') as f:
    Centroids = [[float(num) for num in line.split(',')] for line in f]
num_pores = len(Centroids)

coordinates = [[] for item in range(num_pores)]

with open('inputs/list_boundary_alpha_shape.txt', 'r') as f:
    list_boundary = [int(elem) - 1 for elem in f.read().split(',') if elem]

with open('inputs/list_boundary_voxelized.txt', 'r') as f:
    list_boundary_vox = [int(elem) - 1 for elem in f.read().split(',') if elem]

with open('inputs/Diameters.txt', 'r') as f:
    ...
    while index < len(ordered_pores) - 1:
        ...
        for i in range(len(Holes)):
            ...
            for j in range(len(Holes[i])):
                if is_line([Holes[i][item] for item in [j, next_index(j, 1, len(Holes[i]))]],
                           deg_3_edges_in_cross[vertex]) and is_line(
                    [Holes[i][item] for item in [j, previous_index(j, 1, len(Holes[i]))]],
                    deg_3_edges_in_cross[vertex]):
...
while lst_compute:
    ...
    open_edges = exposed_edges_in_link(vertex, Matrix_out, Centroids)
    ...
    for i in range(len(Holes)):
        priority = []
        for j in range(len(Holes[i])):
            if edges_in_cross and is_line([Holes[i][item] for item in [j, next_index(j, 1, len(Holes[i]))]],
                                          edges_in_cross) and is_line(
                [Holes[i][item] for item in [j, previous_index(j, 1, len(Holes[i]))]], edges_in_cross):
                priority.append(Holes[i][j])
        for j in range(len(Holes[i]) - 2):  # extract priority vertices
            if Holes[i][j] == Holes[i][j + 2]:
                priority = union(priority, Holes[i][j + 1])
...
while True:
    ...
    for v_sub in All:
        ...
        Holes, edges_in_cross = [], []
        open_edges = exposed_edges_in_link(v_sub, Matrix_out, Centroids)
        if len(open_edges) <= 75:
                ...
                if not is_line(edge, candidates_edges[v_sub]):
                    ...
                    for k in mutual_neighbors(edge, Matrix_out):
                        if k not in [v_sub] + Matrix_out[v_sub]:
                            ...
                            for l in Holes[i]:
                                if k not in Matrix_out[l] and not is_line(
                                        [k, l], other_edges) and not is_line([k, l], Holes[i]):
                                    dist = distance_vox(k, l, Centroids, coordinates, voxels, dist_path)
                                    if dist > 1.5 * threshold:
                                        continue
                                    other_edges.append([k, l])
...
while loop_over:
    ...
    if len(vts) < 3 or (len(vts) < 6 and not cycles(edges_of(vts, Matrix_out), minimum_allowed=3)):
        caused_by_football_pores.append(edge)
        loop_over = loop_over[1:]
        continue
    ...
    if len(Holes_1) > 1 and len(Holes_2) > 1:
        ...
        for index in range(len(candidates)):
            ...
            cycle = candidates[index]
            d = sum([is_line(item, cycle) for item in K4])
            if len(cycle) > 3 and sum([is_line(edge, cycle) for edge in edges]) - d >= 2 * len(
                    cycle) - 3:  # hole is already triangulated
                lst_remove.append(index)
            Matrix_temp_sub = loads(dumps(Matrix_out))
            for i in Matrix_temp_sub[edge[0]]:
                if i in cycle:
                ...
                for index_sub in range(len(pos)):
                    if pos[index_sub] == "1":
                        empty_test_sub += [item for item in [[edge[0]] + triangles[index_sub],
                                                             [edge[1]] + triangles[index_sub]] if
                                           not is_line(item, empty_test_sub)]
...
for i in range(num_pores):
    ...
    for j in [item for item in Nei if item > i]:
        ...
        if not intersect([i, j], union(list_boundary, list_boundary_vox)):
            count_interior += 1
            if not is_line([i, j], multi_edges + caused_by_football_pores):
            ...
            if not is_edge_proper_in_triangulation([i, j], triangulation_reconstructed):
                edges_defect_link.append([i, j])
count, count_interior = max(1, count), max(1, count_interior)
count_reconstructable, count_reconstructable_interior = max(1, count_reconstructable), max(1,
                                                                                           count_reconstructable_interior)
...
print("Number of non-properly reconstructed edges around football pores:", len(caused_by_football_pores))
leftovers = [item for item in edges_defect_link if not is_line(item, multi_edges + caused_by_football_pores)]
leftovers_interior = [item for item in edges_defect_link_interior if not is_line(item, multi_edges + caused_by_football_pores)]
...
log_file_detailed.write(
    "\nNumber of non-properly reconstructed edges around football pores: {}".format(len(caused_by_football_pores)))
log_file_detailed.write(
...
while loop_over:
    ...
    if T:
        loop_over = loop_over[1:]
        continue
    problems = [item for item in problems if not is_line([vertex, item], multi_edges + caused_by_football_pores)]
    if not problems:
        loop_over = loop_over[1:]
        continue
    updated = False
    ...
    proper_vertices_initial = [v for v in lst_vertices if
                               is_vertex_proper_in_triangulation(v, triangulation_reconstructed,
                                                                 Matrix_out[v])]
    proper_vertices_initial_int = set_diff(proper_vertices_initial, list_boundary)
    ...
    for K5_current in K5_targeted:
        ...
        proper_edges_initial = [edge for edge in lst_edges if is_edge_proper_in_triangulation(
            edge, triangulation_reconstructed)]
...
for vertex in range(num_pores):
    if vertex in union(list_boundary, list_boundary_vox, ignore):
        continue
    if vertex in union(unique(caused_by_football_pores), unique(multi_edges)):
        continue
...
for i in range(num_pores):
    ...
    for j in [item for item in Nei if item > i]:
        if sublist([i, j], union(list_boundary, list_boundary_vox)):
            continue
        if intersect([i, j], union(unique(caused_by_football_pores), unique(multi_edges))):
            continue
        count += 1
        if not intersect([i, j], union(list_boundary, list_boundary_vox)):
            count_interior += 1
...
print("Number of non-properly reconstructed edges around football pores: {}".format(len(caused_by_football_pores)))
...
log_file_detailed.write("\nNumber of non-properly reconstructed edges around football pores: {}".
                        format(len(caused_by_football_pores)))
...
save_list_of_lists(Matrix_out, "outputs/adjacency_matrix.txt")
save_list(football_pores, "outputs/football_pores.txt")
save_list_of_lists(football_pores_neighbors, "outputs/football_pores_neighbors.txt")
save_list_of_lists(changed_tunnels, "outputs/changed_tunnels.txt")
save_list_of_lists(caused_by_football_pores, "outputs/caused_by_football_pores.txt")
save_list_of_lists(multi_edges, "outputs/multi_edges.txt")
save_list_of_lists(triangulation_reconstructed, "outputs/triangulation_reconstructed.txt")
